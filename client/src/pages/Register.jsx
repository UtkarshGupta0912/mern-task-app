...
function Register() {
  // useState → stores form data, re-renders when changed
  const [form, setForm] = useState({ name: '', email: '', password: '' });
  const [error, setError] = useState('');
  const navigate = useNavigate(); // used to redirect

  // runs on every keystroke — updates the right field
  const handleChange = (e) => {
    setForm({ ...form, [e.target.name]: e.target.value });
    // ...form keeps old values, [e.target.name] updates only the changed field
  };
